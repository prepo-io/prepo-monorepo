...
const deployFunction: DeployFunction = async function ({
  ...
  console.log('Running Collateral deployment script with', deployer, 'as the deployer')
  ...
  if ((await singleStrategyController.getVault()) !== collateral.address) {
    ...
    await sendTxAndWait(await singleStrategyController.setVault(collateral.address))
  }
  if ((await collateral.getStrategyController()) !== singleStrategyController.address) {
    console.log(
      'Connecting SingleStrategyController at',
      singleStrategyController.address,
      'to the Collateral vault...'
    )
    await sendTxAndWait(await collateral.setStrategyController(singleStrategyController.address))
  }
  ...
  if ((await depositHook.getVault()) !== collateral.address) {
    console.log('Connecting Collateral to DepositHook at', depositHook.address, '...')
    await sendTxAndWait(await depositHook.setVault(collateral.address))
  }
  if ((await collateral.getDepositHook()) !== depositHook.address) {
    console.log('Connecting DepositHook at', depositHook.address, 'to the Collateral vault...')
    await sendTxAndWait(await collateral.setDepositHook(depositHook.address))
  }
  ...
  if ((await withdrawHook.getVault()) !== collateral.address) {
    console.log('Connecting Collateral to WithdrawHook at', withdrawHook.address, '...')
  ...
  if ((await collateral.getWithdrawHook()) !== withdrawHook.address) {
    console.log('Connecting WithdrawHook at', withdrawHook.address, 'to the Collateral vault...')
    await sendTxAndWait(await collateral.setWithdrawHook(withdrawHook.address))
  }
  console.log('')
}
