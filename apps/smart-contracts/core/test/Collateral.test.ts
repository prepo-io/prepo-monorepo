...
describe('=> Collateral', () => {
  ...
  let mintCollateralToWithdraw: (minter: SignerWithAddress, amount: BigNumber) => Promise<BigNumber>
  const TEST_APY = 7
  ...
  beforeEach(async () => {
    ...
    collateral = await testCollateralFixture(baseToken.address, treasury.address)
    setupControllerAndStrategy = async (): Promise<void> => {
      await baseToken.mint(deployer.address, MOCK_BASE_TOKEN_SUPPLY)
      strategyController = await singleStrategyControllerFixture(baseToken.address)
      await strategyController.setVault(collateral.address)
      mockStrategy = await mockStrategyFixture(strategyController.address, baseToken.address)
    ...
    sharesMinted = async (amount: BigNumber): Promise<BigNumber> =>
      amount.mul(await collateral.totalSupply()).div(await mockStrategy.totalValue())

    mintCollateralToWithdraw = async (
      minter: SignerWithAddress,
      amount: BigNumber
    ): Promise<BigNumber> => {
      await transferAndApproveForDeposit(minter, collateral.address, amount)
      await collateral.connect(minter).deposit(amount)
      return collateral.balanceOf(minter.address)
    }
  })
  ...
  describe('# totalAssets', () => {
    it('should report sum of latent contract balance and value reported by strategy', async () => {
      ...
      await transferAndApproveForDeposit(user, collateral.address, TEST_ACCOUNT_DEPOSIT_CAP)
      await collateral.connect(user).deposit(TEST_ACCOUNT_DEPOSIT_CAP)
      await baseToken.transfer(collateral.address, TEST_ACCOUNT_DEPOSIT_CAP)
      const collateralBalance = await baseToken.balanceOf(collateral.address)
      const strategyValue = await mockStrategy.totalValue()

      expect(await collateral.totalAssets()).to.eq(collateralBalance.add(strategyValue))
    })
  })
  ...
  describe('# getAmountForShares', () => {
    ...
    it('should return shares at 1:1 ratio if totalSupply is zero', async () => {
      ...
      await setupApyAndBeginning(TEST_APY)
      await baseToken.transfer(collateral.address, TEST_ACCOUNT_DEPOSIT_CAP)
      expect(await baseToken.balanceOf(collateral.address)).to.eq(TEST_ACCOUNT_DEPOSIT_CAP)
      expect(await collateral.totalSupply()).to.eq(0)

      expect(await collateral.getAmountForShares(sharesToQuote)).to.eq(sharesToQuote)
    })
  ...
  describe('# deposit', () => {
    beforeEach(async function () {
      ...
      if (this.currentTest?.title !== "should not call depositHook if a hook isn't configured") {
    ...
    it("should not allow deposits when 'depositsAllowed' is set to false", async () => {
      ...
      await expect(collateral.connect(user).deposit(TEST_ACCOUNT_DEPOSIT_CAP)).revertedWith(
        revertReason('Deposits not allowed')
      )
    })
    ...
    it('should not allow a non-zero deposit too small for a fee', async () => {
      await transferAndApproveForDeposit(user, collateral.address, ethers.BigNumber.from(1))
    ...
    it('should correctly process a deposit with a non-zero initial supply', async () => {
      await transferAndApproveForDeposit(user, collateral.address, TEST_ACCOUNT_DEPOSIT_CAP.div(2))
      await collateral.connect(user).deposit(TEST_ACCOUNT_DEPOSIT_CAP.div(2))
      const fee = calculateFee(TEST_ACCOUNT_DEPOSIT_CAP.div(2), await collateral.getMintingFee())
      const userCTBalanceBefore = await collateral.balanceOf(user.address)
      const totalDepositedBefore = await depositRecord.getGlobalDepositAmount()
      const userNetDepositsBefore = await depositRecord.getNetDeposit(user.address)
      const treasuryBTBalanceBefore = await baseToken.balanceOf(treasury.address)
      const mockStrategyBTBalanceBefore = await baseToken.balanceOf(mockStrategy.address)
      const userBTBalanceBefore = await baseToken.balanceOf(user.address)
      expect(userCTBalanceBefore).to.eq(TEST_ACCOUNT_DEPOSIT_CAP.div(2).sub(fee))
      expect(treasuryBTBalanceBefore).to.eq(fee)
      expect(mockStrategyBTBalanceBefore).to.eq(TEST_ACCOUNT_DEPOSIT_CAP.div(2).sub(fee))
      expect(userBTBalanceBefore).to.eq(0)
      await transferAndApproveForDeposit(user, collateral.address, TEST_ACCOUNT_DEPOSIT_CAP.div(2))
      const expectedShares = await sharesMinted(TEST_ACCOUNT_DEPOSIT_CAP.div(2).sub(fee))
      ...
      expect(await baseToken.balanceOf(treasury.address)).to.eq(treasuryBTBalanceBefore.add(fee))
      expect(await baseToken.balanceOf(mockStrategy.address)).to.eq(
        mockStrategyBTBalanceBefore.add(TEST_ACCOUNT_DEPOSIT_CAP.div(2).sub(fee))
      )
      expect(await baseToken.balanceOf(user.address)).to.eq(userBTBalanceBefore.add(0))
    })

    it('should transfer any latent balance >0 to the strategy', async () => {
      await transferAndApproveForDeposit(user, collateral.address, TEST_ACCOUNT_DEPOSIT_CAP.div(2))
      await baseToken.transfer(collateral.address, TEST_ACCOUNT_DEPOSIT_CAP.div(2))
      expect(await collateral.balanceOf(user.address)).to.eq(0)
      expect(await baseToken.balanceOf(user.address)).to.eq(TEST_ACCOUNT_DEPOSIT_CAP.div(2))
      expect(await baseToken.balanceOf(collateral.address)).to.eq(TEST_ACCOUNT_DEPOSIT_CAP.div(2))
      expect(await baseToken.balanceOf(treasury.address)).to.eq(0)
      expect(await baseToken.balanceOf(mockStrategy.address)).to.eq(0)

      await collateral.connect(user).deposit(TEST_ACCOUNT_DEPOSIT_CAP.div(2))

      const fee = calculateFee(TEST_ACCOUNT_DEPOSIT_CAP, await collateral.getMintingFee())
      expect(await collateral.balanceOf(user.address)).to.eq(TEST_ACCOUNT_DEPOSIT_CAP.sub(fee))
    ...
    it('should return the amount of shares minted', async () => {
      ...
      expect(
        await collateral.connect(user).callStatic.deposit(TEST_ACCOUNT_DEPOSIT_CAP.div(2))
  ...
  describe('# initiateWithdrawal', () => {
    ...
    it("should not allow requesting an amount greater than the account's balance", async () => {
      const sharesMintedForUser = await mintCollateralToWithdraw(user, TEST_ACCOUNT_DEPOSIT_CAP)
      // Add 1 so that the amount being initiated exceeds the account's balance.
      const sharesToRedeem = sharesMintedForUser.add(1)
      expect(await collateral.balanceOf(user.address)).to.be.lt(sharesToRedeem)

      await expect(collateral.connect(user).initiateWithdrawal(sharesToRedeem)).to.be.revertedWith(
    ...
    it('should correctly record a non-zero withdrawal request for the caller', async () => {
      ...
      expect(await collateral.balanceOf(user.address)).to.be.gte(sharesToRedeem)
      const requestOnRecordBefore = await collateral.getWithdrawalRequest(user.address)
      ...
      const recordOnRecordAfter = await collateral.getWithdrawalRequest(user.address)
      expect(recordOnRecordAfter.amount).to.eq(sharesToRedeem)
      expect(recordOnRecordAfter.blockNumber).to.eq(await getLastBlockNumber())
    })

    it('should allow a withdrawal request of zero', async () => {
      const sharesMintedForUser = await mintCollateralToWithdraw(user, TEST_ACCOUNT_DEPOSIT_CAP)
      expect(await collateral.balanceOf(user.address)).to.be.gte(sharesMintedForUser)
      const requestOnRecordBefore = await collateral.getWithdrawalRequest(user.address)
      expect(requestOnRecordBefore.amount).to.eq(0)
      expect(requestOnRecordBefore.blockNumber).to.eq(0)
    ...
    it('should update the block number for a request and keep the amount unchanged if called again with the same amount', async () => {
      ...
      await collateral.connect(user).initiateWithdrawal(firstAmountToRequest)
      const requestOnRecordBefore = await collateral.getWithdrawalRequest(user.address)
      const secondAmountToRequest = sharesMintedForUser
      expect(secondAmountToRequest).to.be.eq(firstAmountToRequest)

      await collateral.connect(user).initiateWithdrawal(secondAmountToRequest)

      const blockFromSecondRequest = await getLastBlockNumber()
      // Verify the block from the second withdrawal request is greater
      expect(blockFromSecondRequest).to.be.gt(requestOnRecordBefore.blockNumber)
      const requestOnRecordAfter = await collateral.getWithdrawalRequest(user.address)
      expect(requestOnRecordAfter.amount).to.eq(requestOnRecordBefore.amount)
      expect(requestOnRecordAfter.blockNumber).to.eq(blockFromSecondRequest)
    })
    ...
    it('should update the block number for a request and change the amount if called again with a greater amount', async () => {
      ...
      expect(blockFromSecondRequest).to.be.gt(requestOnRecordBefore.blockNumber)
      const requestOnRecordAfter = await collateral.getWithdrawalRequest(user.address)
      expect(requestOnRecordAfter.amount).to.eq(requestOnRecordBefore.amount.add(1))
      expect(requestOnRecordAfter.blockNumber).to.eq(blockFromSecondRequest)
    })
    ...
    it('should update the block number for a request and change the amount if called again with a lower amount', async () => {
      ...
      expect(blockFromSecondRequest).to.be.gt(requestOnRecordBefore.blockNumber)
      const requestOnRecordAfter = await collateral.getWithdrawalRequest(user.address)
      expect(requestOnRecordAfter.amount).to.eq(requestOnRecordBefore.amount.sub(1))
      expect(requestOnRecordAfter.blockNumber).to.eq(blockFromSecondRequest)
    })
  })
  ...
  describe('# uninitiateWithdrawal', () => {
    ...
    it("should reset the caller's existing withdrawal request", async () => {
      ...
      expect(requestOnRecordBefore.blockNumber).to.eq(await getLastBlockNumber())
  ...
  describe('# withdraw w/ delayed withdrawal expiry of zero', () => {
    beforeEach(async function () {
      ...
      await collateral.setDepositHook(mockDepositHook.address)
      if (this.currentTest?.title !== "should not call withdrawHook if a hook isn't configured") {
        await collateral.setWithdrawHook(mockWithdrawHook.address)
      }
    })
    ...
    it("should not allow withdrawals when 'withdrawalsAllowed' is set to false", async () => {
      ...
      await expect(collateral.connect(user).withdraw(sharesToRedeem)).revertedWith(
        revertReason('Withdrawals not allowed')
      )
    })
    ...
    it('should correctly process withdrawal amount >0', async () => {
      ...
      const totalDepositsBefore = await depositRecord.getGlobalDepositAmount()
      const netUserDepositsBefore = await depositRecord.getNetDeposit(user.address)
      expect(await collateral.balanceOf(user.address)).to.eq(mintedShares)
      expect(await baseToken.balanceOf(user.address)).to.eq(0)
      expect(await baseToken.balanceOf(collateral.address)).to.eq(0)
      expect(await baseToken.balanceOf(mockStrategy.address)).to.eq(mintedShares)
      const mintingFee = TEST_ACCOUNT_DEPOSIT_CAP.sub(mintedShares)
      expect(await baseToken.balanceOf(treasury.address)).to.eq(mintingFee)
      ...
      const virtualAfterDelay = returnFromMockAPY(TEST_APY, TEST_TIMESTAMP_DELAY, supplyBefore)
      const amountOwed = virtualAfterDelay.mul(sharesToRedeem).div(supplyBefore)
      expect(await depositRecord.getGlobalDepositAmount()).to.eq(
      ...
      const redemptionFee = calculateFee(amountOwed, await collateral.getRedemptionFee())
      const amountReceived = amountOwed.sub(redemptionFee)
      expect(await baseToken.balanceOf(user.address)).to.eq(amountReceived)
      expect(await baseToken.balanceOf(collateral.address)).to.eq(0)
      expect(await baseToken.balanceOf(mockStrategy.address)).to.eq(mintedShares.sub(amountOwed))
      expect(await baseToken.balanceOf(treasury.address)).to.eq(mintingFee.add(redemptionFee))
    })

    it('should call withdrawHook with the correct parameters if a hook is configured', async () => {
      expect(await collateral.getWithdrawHook()).to.eq(mockWithdrawHook.address)
      const beginning = await setupApyAndBeginning(TEST_APY)
      const mintedShares = await mintCollateralToWithdraw(user, TEST_ACCOUNT_DEPOSIT_CAP)
      const supplyBefore = await collateral.totalSupply()
      const mintingFee = TEST_ACCOUNT_DEPOSIT_CAP.sub(mintedShares)
      ...
      const virtualAfterDelay = returnFromMockAPY(TEST_APY, TEST_TIMESTAMP_DELAY, supplyBefore)
      const amountOwed = virtualAfterDelay.mul(sharesToRedeem).div(supplyBefore)
      expect(mockWithdrawHook.hook).to.be.calledWith(user.address, sharesToRedeem, amountOwed)
    })
    ...
    it('should return the amount of Base Token redeemed', async () => {
      ...
      const virtualAfterDelay = returnFromMockAPY(TEST_APY, timePassed, supplyBefore)
      const amountOwed = virtualAfterDelay.mul(sharesToRedeem).div(supplyBefore)
      const redemptionFee = calculateFee(amountOwed, await collateral.getRedemptionFee())
      const amountReceived = amountOwed.sub(redemptionFee)
  ...
  describe('# withdraw w/ non-zero delayed withdrawal expiry', () => {
    ...
    beforeEach(async function () {
      await setupControllerAndStrategy()
      await collateral.setDelayedWithdrawalExpiry(testWithdrawalDelayExpiry)
      await collateral.setDepositHook(mockDepositHook.address)
      if (this.currentTest?.title !== "should not call withdrawHook if a hook isn't configured") {
        await collateral.setWithdrawHook(mockWithdrawHook.address)
      }
    ...
    it('should not allow a withdrawal if it was not initiated beforehand', async () => {
      ...
      const sharesMintedForUser = await mintCollateralToWithdraw(user, TEST_ACCOUNT_DEPOSIT_CAP)
      const sharesToRedeem = sharesMintedForUser
      const requestOnRecord = await collateral.getWithdrawalRequest(user.address)
      expect(requestOnRecord.amount).to.eq(0)
      expect(requestOnRecord.blockNumber).to.eq(0)

      await expect(collateral.connect(user).withdraw(sharesToRedeem)).to.be.revertedWith(
    ...
    it('should not allow a withdrawal when the initiated amount does not match the amount being withdrawn', async () => {
      ...
      await collateral.connect(user).initiateWithdrawal(sharesMintedForUser)
      const requestOnRecord = await collateral.getWithdrawalRequest(user.address)
    ...
    it('should not allow a withdrawal to occur in the same block that it was initiated in', async () => {
      ...
      const sharesMintedForUser = await mintCollateralToWithdraw(user, TEST_ACCOUNT_DEPOSIT_CAP)
      const sharesToRedeem = sharesMintedForUser
      const initiateWithdrawalCallData = collateral.interface.encodeFunctionData(
        'initiateWithdrawal',
        [sharesToRedeem]
      )
      const withdrawCallData = collateral.interface.encodeFunctionData('withdraw', [sharesToRedeem])

      await expect(
        collateral.connect(user).multicall([initiateWithdrawalCallData, withdrawCallData])
      ).to.be.revertedWith(revertReason('Must withdraw in a later block'))
    })
    ...
    it("should not allow a withdrawal to occur if the account's request has expired", async () => {
      ...
      const blocksUntilExpiry = (await collateral.getDelayedWithdrawalExpiry()).toNumber() + 1
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await mineBlocks(ethers.provider as any, blocksUntilExpiry)

      await expect(collateral.connect(user).withdraw(sharesToRedeem)).to.be.revertedWith(
        revertReason('Must withdraw before expiry')
      )
    })
    ...
    it("should correctly process a withdrawal amount > 0 when the account's request expiry deadline equals the current block", async () => {
      ...
      const totalDepositsBefore = await depositRecord.getGlobalDepositAmount()
      const netUserDepositsBefore = await depositRecord.getNetDeposit(user.address)
      expect(await collateral.balanceOf(user.address)).to.eq(sharesMintedForUser)
      expect(await baseToken.balanceOf(user.address)).to.eq(0)
      expect(await baseToken.balanceOf(collateral.address)).to.eq(0)
      expect(await baseToken.balanceOf(mockStrategy.address)).to.eq(sharesMintedForUser)
      const mintingFee = TEST_ACCOUNT_DEPOSIT_CAP.sub(sharesMintedForUser)
      expect(await baseToken.balanceOf(treasury.address)).to.eq(mintingFee)
      const sharesToRedeem = sharesMintedForUser.div(2)
      await collateral.connect(user).initiateWithdrawal(sharesToRedeem)
      const requestOnRecord = await collateral.getWithdrawalRequest(user.address)
      expect(requestOnRecord.amount).to.be.eq(sharesToRedeem)
      const requestBlockExpiry = requestOnRecord.blockNumber.toNumber() + testWithdrawalDelayExpiry
      const blocksUntilExpiry = requestBlockExpiry - (await getLastBlockNumber())
       ...
       */
      const timePassedIncludingDelay = (await getLastTimestamp()) - beginning + TEST_TIMESTAMP_DELAY
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await setNextTimestamp(ethers.provider as any, beginning + timePassedIncludingDelay)
      ...
      expect(await collateral.totalSupply()).to.eq(sharesLeft)
      expect(await collateral.balanceOf(user.address)).to.eq(sharesLeft)
      const virtualAfterDelay = returnFromMockAPY(TEST_APY, timePassedIncludingDelay, supplyBefore)
      const amountOwed = virtualAfterDelay.mul(sharesToRedeem).div(supplyBefore)
      expect(await depositRecord.getGlobalDepositAmount()).to.eq(
        totalDepositsBefore.sub(amountOwed)
      )
    ...
    it('should return the amount of Base Token redeemed', async () => {
       ...
       */
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await mineBlock(ethers.provider as any, beginning + TEST_TIMESTAMP_DELAY)
  ...
  describe('# setDepositsAllowed', () => {
    it('should only be callable by the owner', async () => {
      ...
      await expect(collateral.connect(user).setDepositsAllowed(true)).to.revertedWith(
        revertReason('Ownable: caller is not the owner')
      )
    })
    ...
    it('should emit a DepositsAllowedChanged event', async () => {
      ...
      const depositsAllowedChangedEvent = await getDepositsAllowedChangedEvent(collateral)
  ...
  describe('# setWithdrawalsAllowed', () => {
    it('should only be callable by the owner', async () => {
      ...
      await expect(collateral.connect(user).setWithdrawalsAllowed(true)).to.revertedWith(
        revertReason('Ownable: caller is not the owner')
      )
    })
    ...
    it('should emit a WithdrawalsAllowedChanged event', async () => {
      ...
      const withdrawalsAllowedChangedEvent = await getWithdrawalsAllowedChangedEvent(collateral)
  ...
  describe('# setStrategyController', () => {
    it('should only be callable by the owner', async () => {
      await expect(collateral.connect(user).setStrategyController(user.address)).to.revertedWith(
        revertReason('Ownable: caller is not the owner')
      )
    })

    it('should be settable to a non-zero address', async () => {
      expect(await collateral.getStrategyController()).to.not.eq(user.address)

      await collateral.setStrategyController(user.address)

      expect(await collateral.getStrategyController()).to.eq(user.address)
    })
    ...
    it('should emit a StrategyControllerChanged event', async () => {
      ...
      const strategyControllerChangedEvent = await getStrategyControllerChangedEvent(collateral)
      expect(strategyControllerChangedEvent.controller).to.eq(user.address)
    })
  })

  describe('# setMintingFee', () => {
    it('should only be callable by the owner', async () => {
      await expect(collateral.connect(user).setMintingFee(FEE_LIMIT - 1)).to.revertedWith(
        revertReason('Ownable: caller is not the owner')
      )
    })

    it('should not be settable beyond FEE_LIMIT', async () => {
      await expect(collateral.setMintingFee(FEE_LIMIT + 1)).to.revertedWith(
    ...
    it('should emit a MintingFeeChanged event', async () => {
      ...
      const mintingFeeChangedEvent = await getCollateralMintingFeeChangedEvent(collateral)
  ...
  describe('# setRedemptionFee', () => {
    it('should only be callable by the owner', async () => {
      await expect(collateral.connect(user).setRedemptionFee(FEE_LIMIT - 1)).to.revertedWith(
        revertReason('Ownable: caller is not the owner')
      )
    })

    it('should not be settable beyond FEE_LIMIT', async () => {
      await expect(collateral.setRedemptionFee(FEE_LIMIT + 1)).to.revertedWith(
        revertReason('Exceeds fee limit')
      )
    })
    ...
    it('should emit a RedemptionFeeChanged event', async () => {
      ...
      const redemptionFeeChangedEvent = await getCollateralRedemptionFeeChangedEvent(collateral)
  ...
  describe('# setDelayedWithdrawalExpiry', () => {
    ...
    it('should only be callable by the owner', async () => {
      await expect(
        collateral.connect(user).setDelayedWithdrawalExpiry(testWithdrawalDelayExpiry)
    ...
    it('should be settable to a non-zero value', async () => {
      ...
      await collateral.connect(deployer).setDelayedWithdrawalExpiry(testWithdrawalDelayExpiry)

      expect(await collateral.getDelayedWithdrawalExpiry()).to.eq(testWithdrawalDelayExpiry)
    })
    ...
    it('should emit a DelayedWithdrawalExpiryChanged event', async () => {
      ...
      const delayedWithdrawalExpiryEvent = await getDelayedWithdrawalExpiryChangedEvent(collateral)
      expect(delayedWithdrawalExpiryEvent.expiry).to.eq(testWithdrawalDelayExpiry)
    })
  })
  ...
  describe('# setDepositHook', () => {
    it('should only be callable by the owner', async () => {
      ...
      await expect(collateral.connect(user).setDepositHook(user.address)).to.revertedWith(
        revertReason('Ownable: caller is not the owner')
      )
    })
    ...
    it('should emit a DepositHookChanged event', async () => {
      ...
      const depositHookChangedEvent = await getDepositHookChangedEvent(collateral)
  ...
  describe('# setWithdrawHook', () => {
    it('should only be callable by the owner', async () => {
      ...
      await expect(collateral.connect(user).setWithdrawHook(user.address)).to.revertedWith(
        revertReason('Ownable: caller is not the owner')
      )
    })
    ...
    it('should emit a WithdrawHookChanged event', async () => {
      ...
      const withdrawHookChangedEvent = await getWithdrawHookChangedEvent(collateral)
