...
describe('=> SingleStrategyController', () => {
  ...
  describe('# deposit', () => {
    ...
    it('should only be callable by the vault', async () => {
      await expect(strategyController.connect(user).deposit(TEST_DEPOSIT_AMOUNT)).revertedWith(
        revertReason('Caller is not the vault')
      )
    })
    ...
    it('should deposit to the strategy', async () => {
      ...
      await baseToken.connect(vault).approve(strategyController.address, TEST_DEPOSIT_AMOUNT)

      await strategyController.connect(vault).deposit(TEST_DEPOSIT_AMOUNT)
    ...
    it('should deposit the latent funds in the strategyController', async () => {
      ...
      await baseToken.connect(vault).approve(strategyController.address, TEST_DEPOSIT_AMOUNT.div(2))
      await baseToken.transfer(strategyController.address, TEST_DEPOSIT_AMOUNT.div(2))

      await strategyController.connect(vault).deposit(TEST_DEPOSIT_AMOUNT.div(2))
  ...
  describe('# withdraw', () => {
    ...
    it('should only be callable by the vault', async () => {
      await expect(
        strategyController.connect(user).withdraw(user.address, TEST_WITHDRAWAL_AMOUNT)
    ...
    it('should correctly withdraw funds into the specified account', async () => {
      ...
      await strategyController.connect(vault).withdraw(user.address, TEST_WITHDRAWAL_AMOUNT)
  ...
  describe('# migrate', () => {
    ...
    beforeEach(async function () {
      ...
      await baseToken.transferOwnership(mockStrategy.address)
      if (
        this.currentTest?.title !== 'should set the strategy if one is not already set' &&
        this.currentTest?.title !==
          'should revert if target is zero address and strategy is not already set'
      ) {
        await strategyController.migrate(mockStrategy.address)
        mockStrategy2 = await mockStrategyFixture(strategyController.address, baseToken.address)
    ...
    it('should only be usable by the owner', async () => {
      await expect(strategyController.connect(user).migrate(mockStrategy2.address)).revertedWith(
    ...
    it('should set the strategy if one is not already set', async () => {
      ...
      expect(await strategyController.getStrategy()).to.eq(mockStrategy.address)
    })

    it('should set the controller to use the new strategy', async () => {
      expect(await strategyController.getStrategy()).to.eq(mockStrategy.address)
      expect(await baseToken.allowance(strategyController.address, mockStrategy2.address)).to.eq(0)

      await strategyController.migrate(mockStrategy2.address)

      expect(await strategyController.getStrategy()).to.eq(mockStrategy2.address)
    })
    ...
    it('should set the allowance of the old strategy to 0', async () => {
      expect(await baseToken.allowance(strategyController.address, mockStrategy.address)).to.eq(
        ethers.constants.MaxUint256
      )

      await strategyController.migrate(mockStrategy2.address)

      expect(await baseToken.allowance(strategyController.address, mockStrategy.address)).to.eq(0)
    })
    ...
    it('should send latent funds on the controller to the new strategy', async () => {
      await baseToken.transfer(mockStrategy.address, TEST_DEPOSIT_AMOUNT)
      await baseToken.transfer(strategyController.address, TEST_DEPOSIT_AMOUNT)
      expect(await baseToken.balanceOf(mockStrategy.address)).to.eq(TEST_DEPOSIT_AMOUNT)
      expect(await baseToken.balanceOf(mockStrategy2.address)).to.eq(0)
      expect(await baseToken.balanceOf(strategyController.address)).to.eq(TEST_DEPOSIT_AMOUNT)
    ...
    it('should emit a StrategyMigrated event', async () => {
      ...
      const strategyMigratedEvent = await getStrategyMigratedEvent(strategyController)
      expect(strategyMigratedEvent.oldStrategy).to.eq(mockStrategy.address)
      expect(strategyMigratedEvent.newStrategy).to.eq(mockStrategy2.address)
      expect(strategyMigratedEvent.amount).to.eq(await mockStrategy.totalValue())
    })
  })
  ...
  describe('# totalValue', () => {
    ...
    it('should correctly convey the total debt owed by the strategy', async () => {
      ...
      const expectedValue = returnFromMockAPY(TEST_APY, 1, MOCK_COLLATERAL_SUPPLY)

      expect(await strategyController.totalValue()).to.eq(expectedValue)
    })

    it('should include latent funds within the controller', async () => {
      await baseToken.transfer(strategyController.address, TEST_WITHDRAWAL_AMOUNT)
      await mockStrategy.connect(governance).setApy(TEST_APY)
      const blockNumBefore = await ethers.provider.getBlockNumber()
      const blockBefore = await ethers.provider.getBlock(blockNumBefore)
      // setBeginning will increment timestamp to push totalValue to read 1 second of APY
      ...
      expect(await strategyController.totalValue()).to.eq(expectedValue.add(TEST_WITHDRAWAL_AMOUNT))
    })
  })

  describe('# setVault', () => {
    it('should only be usable by the owner', async () => {
      await expect(strategyController.connect(user).setVault(vault.address)).revertedWith(
        revertReason('Ownable: caller is not the owner')
      )
    })
    ...
    it('should emit a VaultChanged event', async () => {
      ...
      const event = await getSingleStrategyControllerVaultChangedEvent(strategyController)
