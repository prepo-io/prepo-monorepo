...
describe('AcquisitionRoyaleRPShop', () => {
  ...
  const defaultRenameTokenRpPrice = ethers.utils.parseEther('10')
  const defaultRebrandTokenRpPrice = ethers.utils.parseEther('100')
  const defaultReviveTokenRpPrice = ethers.utils.parseEther('1000')
  const defaultEnterpriseRpPrice = ethers.utils.parseEther('1500')
  const renameSupply = 10000
  const rebrandSupply = 1000
  const reviveSupply = 100
  const testMoatThreshold = ethers.utils.parseEther('50')
  ...
  describe('# purchaseConsumable', () => {
    beforeEach(async () => {
      ...
      await royaleConsumables
        .connect(deployer)
        .safeTransferFrom(deployer.address, acquisitionRoyaleRPShop.address, 2, reviveSupply, [])
      await runwayPoints.transfer(user.address, ethers.utils.parseEther('10000'))
    })
