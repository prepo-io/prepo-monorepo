...
describe('AcquistiionRoyaleInternship', () => {
  ...
  const defaultRpPerInternPerDay = ethers.utils.parseEther('5')
  const defaultGlobalRpLimitPerDay = ethers.utils.parseEther('500')
  const defaultClaimDelay = ONE_HOUR * 8 // Default delay is 8 hours

  beforeEach(async () => {
    ;[deployer, user, user2] = await ethers.getSigners()
    runwayPoints = await mockERC20Fixture('Runway Points', 'RP')
    await runwayPoints.connect(deployer).mint(deployer.address, ethers.utils.parseEther('1000000'))
  ...
  describe('# ownerWithdraw', () => {
    const testContractRp = ethers.utils.parseEther('10')
    const testWithdrawalAmount = testContractRp.sub(ethers.utils.parseEther('1'))
    beforeEach(async () => {
  ...
  describe('# setRpPerInternPerDay', () => {
    const testRpPerInternPerDay = defaultRpPerInternPerDay.add(ethers.utils.parseEther('1'))
    it('should only be usable by the owner', async () => {
      expect(await internship.owner()).to.not.eq(user.address)
  ...
  describe('# setGlobalRpLimitPerDay', () => {
    const testGlobalRpLimitPerDay = defaultGlobalRpLimitPerDay.add(ethers.utils.parseEther('1'))
    it('should only be usable by the owner', async () => {
      expect(await internship.owner()).to.not.eq(user.address)
