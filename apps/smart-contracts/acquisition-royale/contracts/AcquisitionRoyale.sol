  ...
  ERC721EnumerableUpgradeable
{
  ...
  function merge(
    ...
    (uint256 _newCallerRpBalance, uint256 _newTargetRpBalance) = _hook.mergeHook(
      _callerId,
      _targetId,
      _burnedId
    );
  ...
  function withdraw(uint256 _enterpriseId, uint256 _amount) external override nonReentrant {
    ...
    // before any RP operations, update actual balances to match virtual balances
    _updateEnterpriseRp(_enterpriseId);
    (uint256 _newRpBalance, uint256 _rpToMint, uint256 _rpToBurn) = _hook.withdrawHook(
      _enterpriseId,
      _amount
    );
  ...
  function getAuctionPrice() public view override returns (uint256) {
    ...
    uint256 _decayPerSecond = (_foundingParameters.startPrice - _foundingParameters.endPrice) /
  ...
  function isEnterpriseImmune(uint256 _enterpriseId) public view override returns (bool) {
    uint256 _acquisitionImmunityEnd = _enterprises[_enterpriseId].acquisitionImmunityStartTime +
      _immunityPeriods.acquisition;
    uint256 _mergerImmunityEnd = _enterprises[_enterpriseId].mergerImmunityStartTime +
      _immunityPeriods.merger;
    uint256 _revivalImmunityEnd = _enterprises[_enterpriseId].revivalImmunityStartTime +
      _immunityPeriods.revival;
    ...
    returns (uint256)
  {
    ...
    uint256 _lastRpUpdateTime = (_enterprises[_enterpriseId].lastRpUpdateTime == 0)
      ? _gameStartTime
      : _enterprises[_enterpriseId].lastRpUpdateTime;
    uint256 _rpPerDay = _passiveRpPerDay.base +
  ...
  function _competeUnchecked(
    ...
    (uint256 _newCallerRpBalance, uint256 _newTargetRpBalance) = _hook.competeHook(
      _callerId,
      _targetId,
      _damage,
      _rpToSpend
  ...
  function _acquireUnchecked(
    ...
    if (_isFundingNative(_nativeSent)) {
       ...
       */
      (uint256 _amountToRecipient, uint256 _amountToTreasury, ) = _acquireCost.updateAndGetCost(
        _callerId,
        _targetId,
        1
      );
       ...
       */
      (uint256 _amountToRecipient, , uint256 _amountToBurn) = _acquireRpCost.updateAndGetCost(
        _callerId,
        _targetId,
        1
      );
    ...
    (uint256 _newCallerRpBalance, uint256 _newTargetRpBalance) = _hook.acquireHook(
      _callerId,
      _targetId,
      _burnedId,
      _nativeSent
