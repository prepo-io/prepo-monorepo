...
contract AcquisitionRoyaleConsumables is ERC1155Supply, IAcquisitionRoyaleConsumables, Ownable {
  ...
  modifier onlyAcquisitionRoyale() {
    require(_msgSender() == _acquisitionRoyale, "caller is not Acquisition Royale");
    _;
  }
  ...
  function setRenameUri(string memory _newRenameUri) external override onlyOwner {
    _renameUri = _newRenameUri;
    ///@dev mandated by ERC-1155 standard
    emit URI(_renameUri, 0);
  }

  function setRebrandUri(string memory _newRebrandUri) external override onlyOwner {
    _rebrandUri = _newRebrandUri;
    ///@dev mandated by ERC-1155 standard
    emit URI(_rebrandUri, 1);
  }

  function setReviveUri(string memory _newReviveUri) external override onlyOwner {
    _reviveUri = _newReviveUri;
    ///@dev mandated by ERC-1155 standard
