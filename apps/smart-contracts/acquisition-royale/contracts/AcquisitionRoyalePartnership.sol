...
contract AcquisitionRoyalePartnership is Ownable {
  ...
  function addPartnership(
    ...
    _purchaseCounts[address(this)][_partnerTokenIds[_partnerToken]] += _supply;
  }
  ...
  function purchase(address _partnerToken, uint256 _quantity) external {
    ...
    require(
      _quantity <= balanceOf(address(this), _partnerTokenIds[_partnerToken]),
      'exceeds supply'
    );
    ...
    _transferFrom(address(this), msg.sender, _partnerTokenIds[_partnerToken], _quantity);
  }
  ...
  function getPartnerId(address _partnerToken) external view returns (uint256) {
    return _partnerTokenIds[_partnerToken];
  }

  function getPartnerPrice(address _partnerToken) external view returns (uint256) {
