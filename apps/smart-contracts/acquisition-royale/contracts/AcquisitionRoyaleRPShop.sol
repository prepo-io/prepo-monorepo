...
contract AcquisitionRoyaleRPShop is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function purchaseConsumable(uint256 _tokenId, uint256 _qty) external nonReentrant {
    ...
    _runwayPoints.safeTransferFrom(msg.sender, owner(), _qty * _tokenPrice);
    _consumables.safeTransferFrom(address(this), msg.sender, _tokenId, _qty, "");
  }
  ...
  function purchaseEnterprise(uint256 _qty) external nonReentrant {
    ...
    _runwayPoints.safeTransferFrom(msg.sender, owner(), _qty * _enterpriseRpPrice);
    for (uint256 i = 0; i < _qty; i++) {
       ...
       */
      uint256 _idToSend = _acquisitionRoyale.tokenOfOwnerByIndex(address(this), 0);
      _acquisitionRoyale.withdraw(
        _idToSend,
        _acquisitionRoyale.getEnterpriseVirtualBalance(_idToSend)
      );
      _acquisitionRoyale.safeTransferFrom(address(this), msg.sender, _idToSend);
    }
  }

  function ownerConsumablesWithdraw(uint256 _tokenId, uint256 _qty) external onlyOwner {
    _consumables.safeTransferFrom(address(this), owner(), _tokenId, _qty, "");
  }

  function ownerEnterpriseWithdraw(uint256 _tokenId, uint256 _qty) external onlyOwner {
    require(_qty > 0, "Quantity cannot be zero");
    if (_qty == 1) {
      _acquisitionRoyale.safeTransferFrom(address(this), owner(), _tokenId);
    } else {
      require(
        _qty <= _acquisitionRoyale.balanceOf(address(this)),
        "Exceeds shop's Enterprise supply"
  ...
  function getConsumables() external view returns (IAcquisitionRoyaleConsumables) {
    return _consumables;
  }
