...
contract FixedAcquireCost is ICost, Ownable {
  ...
  function setAcquisitionFeePercent(uint256 _newAcquisitionFeePercent) external onlyOwner {
    require(_newAcquisitionFeePercent <= PERCENT_DENOMINATOR, "fee exceeds 100%");
    _acquisitionFeePercent = _newAcquisitionFeePercent;
  }
  ...
  {
    uint256 _acquireCostToTreasury = (_fixedCost * _acquisitionFeePercent) / PERCENT_DENOMINATOR;
    _amountToRecipient = _fixedCost - _acquireCostToTreasury;
    (, uint256 _mergeCostToTreasury, ) = _mergeCost.getCost(_callerId, _recipientId);
