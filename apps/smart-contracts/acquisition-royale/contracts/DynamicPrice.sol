...
import {PRBMath, PRBMathUD60x18} from 'prb-math/contracts/PRBMathUD60x18.sol';
...
contract DynamicPrice is IDynamicPrice, Ownable {
  ...
  function updatePrice(uint256 _bumpCount) external override returns (uint256) {
    if (_checkpointTime > 0) {
      ...
      if (_sinceLastCheckpoint >= _timestep) {
        ...
        uint256 _increaseFactor =
          PRBMathUD60x18.powu((PRBMath.SCALE + _increasePercentPerBump), _bumpCountSinceCheckpoint);
        _checkpointPrice = PRBMathUD60x18.mul(_checkpointPrice, _increaseFactor);
        // calculate decrease from time passing
        uint256 _decreaseFactor =
          PRBMathUD60x18.powu((PRBMath.SCALE - _reductionPercentPerSecond), _sinceLastCheckpoint);
        _checkpointPrice = PRBMathUD60x18.mul(_checkpointPrice, _decreaseFactor);
  ...
  function resetCheckpoint(uint256 _newCheckpointPrice) external override onlyOwner {
    _checkpointTime = block.timestamp;
  ...
  function setReductionPercentPerSecond(uint256 _newReductionPercentPerSecond)
    external
    override
    onlyOwner
  {
    require(_newReductionPercentPerSecond <= 1e18, 'reduction exceeds 100%');
  ...
  function getBumpCountSinceCheckpoint() external view override returns (uint256) {
    return _bumpCountSinceCheckpoint;
  }

  function getIncreasePercentPerBump() external view override returns (uint256) {
    return _increasePercentPerBump;
  }

  function getReductionPercentPerSecond() external view override returns (uint256) {
