...
contract CollabLandExtender1155 is ERC721, Ownable {
  ...
  function balanceOf(address account) public view override returns (uint256) {
    ...
    for (uint256 i = 0; i < _projects.length(); i++) {
      ...
      for (uint256 j = 0; j < _projectIds[_currentProject].length(); j++) {
  ...
  function addProjectsAndIds(address[] memory _projectsToAdd, uint256[] memory _idsToAdd)
    external
    onlyOwner
  {
    require(_projectsToAdd.length == _idsToAdd.length, "array lengths must match");
  ...
  function removeProjectsAndIds(address[] memory _projectsToRemove, uint256[] memory _idsToRemove)
    external
    onlyOwner
  {
    require(_projectsToRemove.length == _idsToRemove.length, "array lengths must match");
  ...
  function getIdsForProject(address _project) external view returns (uint256[] memory) {
